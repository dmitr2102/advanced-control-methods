from __future__ import annotations

import sys
from dataclasses import dataclass
from pathlib import Path

from PIL import Image, ImageDraw, ImageFont

PROJECT_ROOT = Path(__file__).resolve().parent
SRC_DIR = PROJECT_ROOT / "src"
if str(SRC_DIR) not in sys.path:
    sys.path.insert(0, str(SRC_DIR))

from controllers import (  # noqa: E402
    AveragedEnergyController,
    CycleEnergyPIDController,
    DirectLyapunovController,
    HarmonicController,
    LyapunovController,
    PositionPIDController,
)
from simulation import PendulumSimulator, SimulationConfig  # noqa: E402
from system import KapitzaPendulumPlant, PendulumParameters  # noqa: E402


ANGLE_TUBE = 0.10
RATE_TUBE = 0.40
HORIZON = 15.0
DT = 1.0 / 240.0


@dataclass
class MethodResult:
    label: str
    color: tuple[int, int, int]
    times: list[float]
    angle_error: list[float]
    convergence_time: float | None
    convergence_error: float | None
    tube_percent: float | None


def make_controllers() -> list[tuple[str, object, tuple[int, int, int]]]:
    return [
        ("Harmonic", HarmonicController(), (38, 98, 170)),
        ("Averaged-energy", AveragedEnergyController(), (212, 100, 36)),
        ("Lyapunov CLF", LyapunovController(), (60, 150, 90)),
        ("Direct Lyapunov", DirectLyapunovController(), (44, 124, 124)),
        ("PID position", PositionPIDController(), (148, 82, 184)),
        ("PID cycle-energy", CycleEnergyPIDController(), (190, 70, 70)),
    ]


def simulate_method(label: str, controller: object, color: tuple[int, int, int]) -> MethodResult:
    params = PendulumParameters(damping=0.25)
    simulator = PendulumSimulator(
        KapitzaPendulumPlant(params),
        SimulationConfig(dt=DT, initial_phi=0.2, initial_phi_dot=0.0, max_abs_action=140.0),
    )
    if hasattr(controller, "reset"):
        controller.reset()

    times: list[float] = [0.0]
    phis: list[float] = [simulator.state.phi]
    phi_dots: list[float] = [simulator.state.phi_dot]

    steps = int(HORIZON / DT)
    for _ in range(steps):
        state = simulator.step(controller)
        times.append(state.time_value)
        phis.append(state.phi)
        phi_dots.append(state.phi_dot)

    convergence_idx = None
    for idx in range(len(times)):
        if all(abs(value) <= ANGLE_TUBE for value in phis[idx:]) and all(abs(value) <= RATE_TUBE for value in phi_dots[idx:]):
            convergence_idx = idx
            break

    if convergence_idx is None:
        convergence_time = None
        convergence_error = None
        tube_percent = None
    else:
        convergence_time = times[convergence_idx]
        convergence_error = abs(phis[convergence_idx])
        tube_percent = 100.0 * convergence_error / ANGLE_TUBE

    return MethodResult(
        label=label,
        color=color,
        times=times,
        angle_error=[abs(value) for value in phis],
        convergence_time=convergence_time,
        convergence_error=convergence_error,
        tube_percent=tube_percent,
    )


def map_x(t: float, left: int, right: int) -> float:
    return left + (t / HORIZON) * (right - left)


def map_y(v: float, top: int, bottom: int, y_max: float) -> float:
    return bottom - (v / y_max) * (bottom - top)


def draw_series(
    draw: ImageDraw.ImageDraw,
    result: MethodResult,
    left: int,
    top: int,
    right: int,
    bottom: int,
    y_max: float,
) -> None:
    points = [
        (map_x(t, left, right), map_y(v, top, bottom, y_max))
        for t, v in zip(result.times, result.angle_error)
    ]
    if len(points) > 1:
        draw.line(points, fill=result.color, width=3)

    if result.convergence_time is not None and result.convergence_error is not None:
        cx = map_x(result.convergence_time, left, right)
        cy = map_y(result.convergence_error, top, bottom, y_max)
        draw.ellipse((cx - 5, cy - 5, cx + 5, cy + 5), fill=result.color, outline=(255, 255, 255), width=1)
        draw.line((cx, top, cx, bottom), fill=result.color, width=1)


def main() -> None:
    results = [simulate_method(label, controller, color) for label, controller, color in make_controllers()]

    width, height = 1600, 980
    image = Image.new("RGB", (width, height), (245, 244, 240))
    draw = ImageDraw.Draw(image)
    font = ImageFont.load_default()

    title = "Angle-error comparison for tuned Kapitza pendulum controllers"
    subtitle = "Absolute angle error |phi(t)| over 15 s. Markers show the first time after which the state stays inside the target tube."
    draw.text((40, 18), title, fill=(25, 40, 78), font=font)
    draw.text((40, 38), subtitle, fill=(70, 70, 78), font=font)

    plot_rect = (70, 90, 1090, 880)
    info_rect = (1130, 90, 1550, 880)
    x0, y0, x1, y1 = plot_rect
    draw.rounded_rectangle(plot_rect, radius=18, fill=(252, 250, 245), outline=(205, 205, 200), width=2)
    draw.rounded_rectangle(info_rect, radius=18, fill=(252, 250, 245), outline=(205, 205, 200), width=2)

    left = x0 + 70
    top = y0 + 40
    right = x1 - 22
    bottom = y1 - 55
    y_max = max(0.22, max((max(result.angle_error) for result in results), default=0.22)) * 1.08

    draw.line((left, bottom, right, bottom), fill=(90, 90, 90), width=2)
    draw.line((left, top, left, bottom), fill=(90, 90, 90), width=2)

    for idx in range(6):
        t_value = HORIZON * idx / 5.0
        x = map_x(t_value, left, right)
        draw.line((x, top, x, bottom), fill=(228, 228, 228), width=1)
        draw.text((x - 10, bottom + 10), f"{t_value:.0f}", fill=(85, 85, 90), font=font)

    for idx in range(6):
        value = y_max * idx / 5.0
        y = map_y(value, top, bottom, y_max)
        draw.line((left, y, right, y), fill=(228, 228, 228), width=1)
        draw.text((x0 + 10, y - 7), f"{value:.2f}", fill=(85, 85, 90), font=font)

    tube_y = map_y(ANGLE_TUBE, top, bottom, y_max)
    draw.line((left, tube_y, right, tube_y), fill=(140, 140, 140), width=2)
    draw.text((right - 155, tube_y - 18), "tube: |phi| = 0.10 rad", fill=(90, 90, 90), font=font)

    for result in results:
        draw_series(draw, result, left, top, right, bottom, y_max)

    draw.text((right - 20, bottom + 28), "t [s]", fill=(55, 55, 60), font=font)
    draw.text((x0 + 10, top - 18), "|phi| [rad]", fill=(55, 55, 60), font=font)

    legend_x = x0 + 48
    legend_y = y0 + 12
    for idx, result in enumerate(results):
        lx = legend_x + idx * 160
        draw.line((lx, legend_y + 8, lx + 28, legend_y + 8), fill=result.color, width=3)
        draw.text((lx + 36, legend_y), result.label, fill=(55, 55, 60), font=font)

    info_x0, info_y0, info_x1, info_y1 = info_rect
    draw.text((info_x0 + 18, info_y0 + 16), "Convergence summary", fill=(25, 40, 78), font=font)
    draw.text((info_x0 + 18, info_y0 + 36), "Tube definition:", fill=(55, 55, 60), font=font)
    draw.text((info_x0 + 18, info_y0 + 54), "|phi| <= 0.10 rad, |phi_dot| <= 0.40 rad/s", fill=(55, 55, 60), font=font)
    draw.text((info_x0 + 18, info_y0 + 72), "Tube % = 100 * |phi(t_conv)| / 0.10", fill=(55, 55, 60), font=font)

    row_y = info_y0 + 112
    for result in results:
        draw.line((info_x0 + 18, row_y + 8, info_x0 + 42, row_y + 8), fill=result.color, width=3)
        draw.text((info_x0 + 52, row_y), result.label, fill=(40, 40, 45), font=font)
        row_y += 22
        if result.convergence_time is None or result.tube_percent is None:
            draw.text((info_x0 + 52, row_y), "No convergence by 15 s", fill=(100, 45, 45), font=font)
        else:
            draw.text((info_x0 + 52, row_y), f"t_conv = {result.convergence_time:.3f} s", fill=(55, 55, 60), font=font)
            row_y += 18
            draw.text((info_x0 + 52, row_y), f"tube usage = {result.tube_percent:.1f}%", fill=(55, 55, 60), font=font)
        row_y += 34

    output_path = PROJECT_ROOT / "figures" / "tuned_controller_angle_error_comparison_15s.png"
    output_path.parent.mkdir(exist_ok=True)
    image.save(output_path)
    print(f"Saved comparison figure to: {output_path}")
    for result in results:
        if result.convergence_time is None or result.tube_percent is None:
            print(f"{result.label}: no convergence by {HORIZON:.1f} s")
        else:
            print(
                f"{result.label}: t_conv = {result.convergence_time:.3f} s, "
                f"tube usage = {result.tube_percent:.1f}%"
            )


if __name__ == "__main__":
    main()
