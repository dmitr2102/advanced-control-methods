...
class ObstacleCasadiMPCController:
    ...
    def solve(self, env: ObstacleRaceEnv) -> CasadiMPCPlan:
        try:
            import casadi as ca
        ...
        for k in range(n):
            ...
            cost += _obstacle_penalty(ca, env, xk[0], xk[1])
...
def solve_obstacle_casadi_mpc(
    env: ObstacleRaceEnv,
    horizon: int = 24,
    track_margin: float = 1.20,
) -> CasadiMPCPlan:
    return ObstacleCasadiMPCController(horizon=horizon, track_margin=track_margin).solve(env)
...
def _obstacle_penalty(ca, env: ObstacleRaceEnv, s, lateral):
    penalty = 0.0
    for obstacle in env.obstacles:
        clearance_s = obstacle.half_s + 3.2
        clearance_e = obstacle.half_lateral + 1.75
        normalized = (
            ((s - obstacle.s) / clearance_s) ** 2
            + ((lateral - obstacle.lateral_center) / clearance_e) ** 2
        )
        violation = ca.fmax(0.0, 1.0 - normalized)
        close = ca.exp(-2.0 * normalized)
        penalty += 220000.0 * violation * violation + 9500.0 * close
    return penalty
...
def _one_step_safe(env: RaceCarEnv, control: Control) -> bool:
    ...
    for _ in range(env.substeps):
        ...
        if env.slipping(state) or not env.track.inside(state.x, state.y):
            return False
        if hasattr(env, "collides_with_obstacle") and env.collides_with_obstacle(state.x, state.y):
            return False
    return True
